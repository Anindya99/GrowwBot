...
const getInvestments= async(token,userId,type)=>{
    try{
        let response= await fetch(`/api/v1/invest/user/${type}/${userId}`,{
            method: "GET",
            headers:{
...
const investStock= async(token,user,stock,name,type,quantity,total)=>{
    try{
...
export {investStock,getInvestments};